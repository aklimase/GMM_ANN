...
#        setup_curves_compare(site, scaler, workinghome, foldername, siteparams, ANN_list, vref, pre_scatter = np.asarray([0]), obs_scatter = np.asarray([0]), dist_scatter = np.asarray([0]), mag_scatter = np.asarray([0]))
    ...
    plt.xlim([1,np.log10(250.)])
    plt.ylim([2.8,5.0])
 
    cp = plt.contourf(X, Y, Z)
    plt.contour(X, Y, Z, colors='k')#, levels = 6)
    plt.clabel(cp, colors = 'k', fmt = '%2.1f',fontsize=12, inline=True, inline_spacing=1)
...
def plot_mag_curves_top10ANN(preln, mag, dist, workinghome,title):
    ...
    for k in range(10):
        for i in range(len(dist)):
            pre = (preln[k*5+i]*np.log10(np.e)).flatten()
...
#X, Y = np.meshgrid(Rlist, mlist)
#if site == '5coeff':
#    ME_test_predicted = a1 + a2*Y + a3*(8.5-Y)**2. + a4*np.log(X) + a5*X + bias
#else:
